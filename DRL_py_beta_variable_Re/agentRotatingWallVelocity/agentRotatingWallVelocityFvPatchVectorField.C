    ...
    if (t >= start_time_)
    {
        ...
        if (steps_remaining && update_omega_)
        {
            ...
            if (Pstream::master()) //only run on the master
            {
                ...
                torch::Tensor log_alpha_tensor = gauss_parameters[0][0];
                torch::Tensor log_beta_tensor = gauss_parameters[0][1];
                scalar alpha = std::min(std::max(std::exp(log_alpha_tensor.item<double>()),0.5),40.0);
                scalar beta = std::min(std::max(std::exp(log_beta_tensor.item<double>()),0.5),40.0);
