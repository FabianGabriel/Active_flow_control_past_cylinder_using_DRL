    ...
    if (t >= start_time_)
    {
        ...
        if (steps_remaining && update_omega_)
        {
            ...
            if (Pstream::master()) //only run on the master
            {
                ...
                std::cout << "size: " << size << "\n";
                scalar omega_pre_tanh;
                if (train_)
                {
                    // sample from Gaussian distribution during training
                    omega_pre_tanh = at::normal(mean_tensor, log_std_tensor.exp()).item<double>();
                }
                else
                {
                    // use expected (mean) angular velocity
                    omega_pre_tanh = mean_tensor.item<double>();
                }
                // squeeze the distribution
                scalar omega_tanh;
                omega_tanh = Foam::tanh(omega_pre_tanh);
                // rescale to actionspace
                omega_ = omega_tanh * abs_omega_max_;
                // save trajectory
                scalar mean = Foam::tanh(mean_tensor.item<double>());
                ...
                scalar log_p = -((omega_pre_tanh - mean) * (omega_pre_tanh - mean)) / (2.0*var) - log_std - log(sqrt(2.0*M_PI)) - log(max(min((1 - omega_tanh * omega_tanh),1),0) + 0.00001);
                mean *= abs_omega_max_;
